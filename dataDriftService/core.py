...
class DataDriftService(MonitoringInterface):
    def __init__(self,model_id:str,reference_data:pd.DataFrame,current_data:pd.DataFrame):
    ...
    def get_report(self)->DriftReport:
        ...
        result = DriftReport(model_id=self.model_id,report_html=report_html,timestamp=datetime.now())
        return result
